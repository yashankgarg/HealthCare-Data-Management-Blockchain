...
class Blockchain:
    ...
    def verify(self):
        ...
        response = requests.get(f'http://127.0.0.1:5000/get_b')
        if response.status_code == 200:
        ...
        response = requests.get(f'http://127.0.0.1:5000/get_l')
        if response.status_code == 200:
...
def get_b():
    ...
    response = requests.get(f'http://127.0.0.1:5000/get_h')
    if response.status_code == 200:
...
def get_l():
    ...
    #requesting node(miner in this case) to send h value
    response = requests.get(f'http://127.0.0.1:5000/get_s')
    if response.status_code == 200:
